...
octanol-water partition coefficient ... Ready_Biodeg
"""
...
### Concatenate data
Ready_Biodeg = pd.concat([train_df[["Canonical_QSARr", "Ready_Biodeg"]],
                 test_df[["Canonical_QSARr", "Ready_Biodeg"]]], ignore_index = True)
Ready_Biodeg['Ready_Biodeg'] = pd.to_numeric(Ready_Biodeg['Ready_Biodeg'])

### Calculate features
nms = [x[0] for x in Descriptors._descList]
calc = MoleculeDescriptors.MolecularDescriptorCalculator(nms)
for i in range(len(Ready_Biodeg)):
    try:
        descrs = calc.CalcDescriptors(Chem.MolFromSmiles(Ready_Biodeg.iloc[i, 0]))
        for x in range(len(descrs)):
            Ready_Biodeg.at[i, str(nms[x])] = descrs[x]
    except:
        for x in range(len(descrs)):
            Ready_Biodeg.at[i, str(nms[x])] = 'NaN'    
            
Ready_Biodeg = Ready_Biodeg.dropna()

### Training & Test Datasets
X = Ready_Biodeg.drop(columns=["Canonical_QSARr", "Ready_Biodeg"])
y = Ready_Biodeg[["Ready_Biodeg"]]
...
tpot.export('tpot_Ready_Biodeg_pipeline.py')
print(datetime.datetime.now())
