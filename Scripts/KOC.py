...
octanol-water partition coefficient ... LogKOC
"""
...
### Concatenate data
LogKOC = pd.concat([train_df[["Canonical_QSARr", "LogKOC"]],
                 test_df[["Canonical_QSARr", "LogKOC"]]], ignore_index = True)
LogKOC['LogKOC'] = pd.to_numeric(LogKOC['LogKOC'])

### Calculate features
nms = [x[0] for x in Descriptors._descList]
calc = MoleculeDescriptors.MolecularDescriptorCalculator(nms)
for i in range(len(LogKOC)):
    try:
        descrs = calc.CalcDescriptors(Chem.MolFromSmiles(LogKOC.iloc[i, 0]))
        for x in range(len(descrs)):
            LogKOC.at[i, str(nms[x])] = descrs[x]
    except:
        for x in range(len(descrs)):
            LogKOC.at[i, str(nms[x])] = 'NaN'    
            
LogKOC = LogKOC.dropna()

### Training & Test Datasets
X = LogKOC.drop(columns=["Canonical_QSARr", "LogKOC"])
y = LogKOC[["LogKOC"]]
...
tpot.export('tpot_LogKOC_pipeline.py')
print(datetime.datetime.now())
