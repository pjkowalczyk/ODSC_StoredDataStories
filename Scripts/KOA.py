...
octanol-water partition coefficient ... LogKOA
"""
...
### Concatenate data
LogKOA = pd.concat([train_df[["Canonical_QSARr", "LogKOA"]],
                 test_df[["Canonical_QSARr", "LogKOA"]]], ignore_index = True)
LogKOA['LogKOA'] = pd.to_numeric(LogKOA['LogKOA'])

### Calculate features
nms = [x[0] for x in Descriptors._descList]
calc = MoleculeDescriptors.MolecularDescriptorCalculator(nms)
for i in range(len(LogKOA)):
    try:
        descrs = calc.CalcDescriptors(Chem.MolFromSmiles(LogKOA.iloc[i, 0]))
        for x in range(len(descrs)):
            LogKOA.at[i, str(nms[x])] = descrs[x]
    except:
        for x in range(len(descrs)):
            LogKOA.at[i, str(nms[x])] = 'NaN'    
            
LogKOA = LogKOA.dropna()

### Training & Test Datasets
X = LogKOA.drop(columns=["Canonical_QSARr", "LogKOA"])
y = LogKOA[["LogKOA"]]
...
tpot.export('tpot_LogKOA_pipeline.py')
print(datetime.datetime.now())
