...
summary(lm(Predicted ~ LogP, data = data2plot))

p <-
  ggplot(data2plot, aes(LogP, Predicted)) +
...
summary(lm(Predicted ~ LogKOC, data = data2plot))

p <-
  ggplot(data2plot, aes(LogKOC, Predicted)) +
  geom_point(colour = "blue", size = 2) +
  coord_equal() +
  # xlim(c(0, 3.5)) + ylim(c(0, 3.5)) +
  geom_smooth(method = 'lm') +
