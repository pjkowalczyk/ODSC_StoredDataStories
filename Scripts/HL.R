...
## training data
train <-
  read.csv('cache/TR_HL_441_descrs.csv',
           header = TRUE,
           stringsAsFactors = FALSE) %>%
  select(-X,-CAS,-ROMol,-SMILES,-ID) %>%
  select(HL, everything()) %>%
  na.omit()
...
## test data
test <-
  read.csv('cache/TST_HL_150_descrs.csv',
           header = TRUE,
           stringsAsFactors = FALSE) %>%
  select(-X,-CAS,-ROMol,-SMILES,-ID) %>%
  select(HL, everything()) %>%
...
summary(lm(Predicted ~ HL, data = data2plot))

p <-
  ggplot(data2plot, aes(HL, Predicted)) +
