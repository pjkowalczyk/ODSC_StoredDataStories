...
## training data
train <-
  read.csv('cache/TR_KOA_202_descrs.csv',
           header = TRUE,
           stringsAsFactors = FALSE) %>%
  select(-X,-CAS,-ROMol,-SMILES,-ID) %>%
  select(LogKOA, everything()) %>%
  na.omit()
...
## test data
test <-
  read.csv('cache/TST_KOA_68_descrs.csv',
           header = TRUE,
           stringsAsFactors = FALSE) %>%
  select(-X,-CAS,-ROMol,-SMILES,-ID) %>%
  select(LogKOA, everything()) %>%
...
summary(lm(Predicted ~ LogKOA, data = data2plot))

p <-
  ggplot(data2plot, aes(LogKOA, Predicted)) +
